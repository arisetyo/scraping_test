...
CREATE SCHEMA IF NOT EXISTS maven;
...
DO $$ BEGIN
    CREATE TYPE maven.source_type_enum AS ENUM ('social', 'web');
...
CREATE TABLE IF NOT EXISTS maven.scraped_posts (
    id              UUID            PRIMARY KEY DEFAULT gen_random_uuid(),

    -- Source identification
    source          TEXT            NOT NULL,           -- "x", "detik_health", etc.
    source_type     maven.source_type_enum NOT NULL,
...
CREATE OR REPLACE VIEW maven.pending_analysis AS
    SELECT id, source, source_type, keyword, post_id, text, lang, scraped_at
    FROM maven.scraped_posts
    WHERE sentiment_label IS NULL
    ORDER BY scraped_at ASC;

COMMENT ON VIEW maven.pending_analysis IS
...
CREATE OR REPLACE VIEW maven.sentiment_summary AS
    SELECT
        keyword,
        source,
        COUNT(*)                                            AS total,
...
COMMENT ON VIEW maven.sentiment_summary IS
